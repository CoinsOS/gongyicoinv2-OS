...
bool BitcoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 BTC, 1 GongYiCoin
        switch(rcp.txnTypeInd)
        {
            ...
            case TXT_BTC_TO_ANON:
                formatted.append(tr("<b>%1</b> to GONGYICOIN %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> GONGYICOIN, ring size %2 to GONGYICOIN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_BTC:
                formatted.append(tr("<b>%1</b> GONGYICOIN, ring size %2 to GYC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::SCR_AmountWithFeeExceedsBitcoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your gongyicoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
QVariantMap BitcoinBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with GongYiCoin prefix.");
            return result;
        }
